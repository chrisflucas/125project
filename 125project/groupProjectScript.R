...
hiv_tibble <- hiv_tibble[,-c(2:12)]
hiv_tibble <- hiv_tibble[rowSums(is.na(hiv_tibble)) != ncol(hiv_tibble)-1,]
colnames(hiv_tibble)[1] <- "Country"
...
hiv_tibble <- hiv_tibble[-24]
hiv_tibble <- hiv_tibble[-24]
data("countryExData")

hiv_tibble <- left_join(hiv_tibble, countryExData, by="Country")
hiv_tibble <- arrange(hiv_tibble, EPI_regions)
ggplot(hiv_tibble, aes(x=log(GDP_capita.MRYA), y=mean_hiv_rate, size=Population2005, color=EPI_regions))+geom_point()
